...
1. **Variable Dependiente:** Nivel de comunicación y seguimiento de los padres respecto al desempeño estudiantil
...
1. **Enfoque Incremental por Módulos**
...
1. **Diseño e implementación de mecanismos de logging** para recolectar datos relevantes
1. **Creación de instancias de prueba** que simulan diferentes perfiles de usuario
1. **Simulación de comportamientos** basados en patrones de uso esperados
1. **Medición y análisis** de los resultados obtenidos
...
2. **Sistema de Logging y Recolección de Datos**
...
- auth\_logs : Registra eventos relacionados con la autenticación!
- access\_logs : Registra accesos a los diferentes módulos del sistema  
- file\_uploads : Registra cargas de archivos y su procesamiento
...
3. **Métricas Generales del Sistema**
...
1. **Métricas del Módulo de Autenticación**
...
ˊ Logins Exitosos × 100

Tasa de Exito =

Totalde Intentos

**Fuente de Datos:** Tabla  auth\_logs , campos  tipo\_evento  ('login\_exitoso', 'login\_fallido')
...
**Definición:** Duración promedio de las sesiones de usuario, medida desde el login hasta el logout. **Fórmula:**

∑(TimestampLogout − TimestampLogin) Tiempo Promedio =

Nuˊmero de Sesiones

**Fuente de Datos:** Tabla  auth\_logs , timestamps de eventos 'login\_exitoso' y 'logout'
...
2. **Métricas del Módulo de Datos Académicos**
...
Nuˊmero de Consultas Frecuencia =

(Dˊıas delPerˊıodo/7) **Fuente de Datos:** Tabla  access\_logs  donde  modulo = 'calificaciones' 
...
(Dˊıas delPerˊıodo/7) **Fuente de Datos:** Tabla  access\_logs  donde  modulo = 'asistencia' 
...
3. **Métricas del Módulo de Mensajería**
...
Conversaciones con>1 Emisor Porcentaje Bidireccional = × 100 TotalConversaciones
...
**Definición:** Tiempo promedio que tarda un usuario en responder a un mensaje recibido. **Fórmula:**

∑(TimestampRespuesta − TimestampMensaje Original) Tiempo Promedio =

Nuˊmero de Respuestas
...
4. **Validación del Módulo de Autenticación**
...
![M1: Tasa de Éxito en Inicios de Sesión por Instancia](doc/tesis/graficos/M1_tasa_exito_login.png)

*Figura 6.1: Tasa de Éxito en Inicios de Sesión por Instancia*
...
![M2: Tiempo de Sesión Activa por Instancia](doc/tesis/graficos/M2_tiempo_sesion.png)

*Figura 6.2: Tiempo de Sesión Activa por Instancia*
...
1. **Tiempos de sesión diferenciados:** Reflejan adecuadamente los diferentes patrones de uso esperados
1. **Sin fallos de autenticación:** El sistema demuestra ser confiable y estable
...
5. **Validación del Módulo de Datos Académicos**
1. **Métricas Específicas**
...
![M3: Frecuencia de Consulta de Calificaciones por Instancia](doc/tesis/graficos/M3_frecuencia_consulta.png)

*Figura 6.3: Frecuencia de Consulta de Calificaciones por Instancia*
...
![M4: Frecuencia de Consulta de Asistencias por Instancia](doc/tesis/graficos/M4_frecuencia_asistencia.png)

*Figura 6.4: Frecuencia de Consulta de Asistencias por Instancia*
...
1. **Consultas equilibradas:** Los usuarios acceden tanto a calificaciones como a asistencias según sus necesidades
1. **Frecuencias realistas:** Los valores obtenidos (4.67 consultas/semana para padres activos) son consistentes con comportamientos reales de seguimiento académico
...
6. **Validación del Módulo de Mensajería**
1. **Métricas Específicas**
...
![M5: Mensajes Enviados por Instancia](doc/tesis/graficos/M5_mensajes_por_emisor.png)

*Figura 6.5: Mensajes Enviados por Instancia*
...
![M5: Mensajes y Emisores por Conversación](doc/tesis/graficos/M5_mensajes_por_conversacion.png)

*Figura 6.6: Mensajes y Emisores por Conversación*
...
![M6: Tiempo de Respuesta a Mensajes por Rol](doc/tesis/graficos/M6_tiempo_respuesta.png)

*Figura 6.7: Tiempo de Respuesta a Mensajes por Rol*
...
1. **Tiempos de respuesta diferenciados:** Los docentes responden más rápido, lo cual favorece una comunicación ágil
1. **Patrones de uso realistas:** El volumen de mensajes (13 en 2 semanas) es consistente con una comunicación académica efectiva
...
2. **Diferenciación Clara de Perfiles de Usuario**

Los datos demuestran que el sistema permite diferenciar claramente entre perfiles de usuario: *Tabla 6.8: Comparativo de Comportamientos por Instancia*
...
3. **Eficiencia de la Comunicación**
...
8. **Conclusiones de la Validación**
1. **Logros del Sistema**
...
1. **Seguimiento académico efectivo:** Los padres activos consultan calificaciones con una frecuencia de 4.67 veces/semana
1. **Comunicación bidireccional completa:** 100% de las conversaciones involucran a ambas partes
1. **Adaptabilidad a diferentes perfiles:** El sistema permite tanto patrones de uso intensivos como esporádicos
...
1. **Variable Dependiente:** Se evidencia un aumento en la comunicación y seguimiento académico por parte de los padres
...
1. **Adopción por padres reactivos:** Estrategias específicas para aumentar la frecuencia de consulta de calificaciones
1. **Integración entre módulos:** Potenciar la relación entre consulta de datos y comunicación
4. **Impacto Educativo**
