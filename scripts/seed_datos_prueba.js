...
async function main() {
  ...
  try {
    ...
    let nivelGrado = await prisma.nivelGrado.findUnique({
      where: {
        nivel_grado: {
    ...
    for (const calificacionData of calificaciones) {
      // Determinar el curso y componente
      let cursoId, estructuraId;
      
      if (calificacionData.fecha_evaluacion <= new Date('2025-03-15')) {
        // Calificaciones de Matemáticas
        cursoId = cursoMatematicas ? cursoMatematicas.id : cursosCreados.find(c => c.codigo_curso === 'CP3001')?.id;
        
        if (calificacionData.fecha_evaluacion.getDate() === 15) {
          // Es el examen
          estructuraId = estructuraEvaluacionCreada.find(e => e.nombre_item === 'Examen')?.id;
        } else if ([5, 12, 19].includes(calificacionData.fecha_evaluacion.getDate())) {
          // Es participación
          estructuraId = estructuraEvaluacionCreada.find(e => e.nombre_item === 'Participación')?.id;
        } else if ([8, 22].includes(calificacionData.fecha_evaluacion.getDate())) {
          // Es revisión de cuaderno
          estructuraId = estructuraEvaluacionCreada.find(e => e.nombre_item === 'Revisión de Cuaderno')?.id;
        } else if ([10, 17].includes(calificacionData.fecha_evaluacion.getDate())) {
          // Es revisión de libro
          estructuraId = estructuraEvaluacionCreada.find(e => e.nombre_item === 'Revisión de Libro')?.id;
        }
      } else {
        // Calificaciones de Comunicación
        cursoId = cursosCreados.find(c => c.codigo_curso === 'CP3002')?.id;
        
        if (calificacionData.fecha_evaluacion.getDate() === 16) {
          // Es el examen
          estructuraId = estructuraEvaluacionCreada.find(e => e.nombre_item === 'Examen')?.id;
        } else if ([6, 13, 20].includes(calificacionData.fecha_evaluacion.getDate())) {
          // Es participación
          estructuraId = estructuraEvaluacionCreada.find(e => e.nombre_item === 'Participación')?.id;
        }
      }
      
      if (cursoId && estructuraId && estudiante) {
        // Verificar si ya existe
        const calificacionExistente = await prisma.evaluacion.findFirst({
          where: {
            estudiante_id: estudiante.id,
            curso_id: cursoId,
            estructura_evaluacion_id: estructuraId,
            fecha_evaluacion: calificacionData.fecha_evaluacion
          }
        });
        
        if (!calificacionExistente) {
          const calificacion = await prisma.evaluacion.create({
            data: {
              ...calificacionData,
              estudiante_id: estudiante.id,
              curso_id: cursoId,
              estructura_evaluacion_id: estructuraId,
              registrado_por: docente ? docente.id : null
            }
          });
          
          // Convertir calificación numérica a letra
          let calificacionLetra;
          if (calificacion.calificacion_numerica >= 18) calificacionLetra = 'AD';
          else if (calificacion.calificacion_numerica >= 14) calificacionLetra = 'A';
          else if (calificacion.calificacion_numerica >= 11) calificacionLetra = 'B';
          else calificacionLetra = 'C';
          
          // Actualizar con la letra correspondiente
          await prisma.evaluacion.update({
            where: { id: calificacion.id },
            data: { calificacion_letra: calificacionLetra }
          });
          
          console.log(`✅ Calificación creada: ${calificacion.calificacion_numerica} (${calificacionLetra})`);
        }
      }
    }
    ...
    for (const asistenciaData of asistencias) {
      ...
      if (!asistenciaExistente) {
        ...
        console.log(`✅ Asistencia creada: ${asistencia.fecha} - ${asistencia.estado}`);
      }
    }
